                    ...
                    opxi2.kue.Job.get( id, function( err, job ){
                        ...
                        if( job.data.error ) {
                            if( !self.break_on_fail /*self.ignore_on_fail*/ ) {
                                console.log( 'Job task '+job.type+'(%d) completed but with fail data ', job.id );
...
//                                return self.failed( job.data );
                                ...
                                console.log( 'Job task '+job.type+'(%d) failed with data ', job.id );
                                return self.failed( job.data );
                            }
                        } else {
                            console.log( 'Job task '+job.type+'(%d) completed with data %j', job.id );
                            return self.completed( job.data );
                        }
                    });
                }
            ...
            if( retries == 1 ) {
                my_job.on( 'failed', function() {
                    console.log( "job_task failed ??? ", self.name, my_job.id );
    ...
    registerCancel: function( jobId ) {
        if( jobId ) {
            this.on( 'cancel', function () {
                console.log( "Task %s(%s) of flow %s create job %s", this.name, jobId, this.flowId, 'cancel-'+this.name );
                opxi2.taskq.create( 'cancel-' + this.name, { title: 'Flow '+this.flowId+' canceled job ' + jobId, job_id: jobId } ).save();
            }.bind( this ) );
        } else {
            console.log( "Cannot register on task's cancel, saved job has no id ", this.flowId );
        }
    },
